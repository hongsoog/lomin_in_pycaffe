...
class DetectionDemo(object):
    ...
    # --------------------------------
    def __init__(self, global_cfg, weight, is_recognition=False):
        ...
        self.score_threshold = self.cfg.TEST.SCORE_THRESHOLD
    ...
    def run_on_pil_image(self, image_origin):
        ...
        cur_prediction = self.compute_prediction(pil_image)
        cur_prediction = self.filter_by_score(cur_prediction)
        cur_prediction = cur_prediction.resize(image_origin.size)
        result = self.parse_result(cur_prediction)
        return result
    ...
    def compute_prediction(self, image):
        ...
        # use another image from the training set solve the problems
        with torch.no_grad():
            #pred = self.model(image_list)
            print(f"image_list.tensors[0].shape, {image_list.tensors[0].shape}")
            print(f"image_list.image_sizes): {image_list.image_sizes}")
            torchinfo.summary(self.model.backbone,
                              #input_size=(1, 3, 480, 561),
                              batch_dim=1,
    ...
    # --------------------------------
    def filter_by_score(self, predictions):
        filter_threshold = predictions.get_field('scores') > self.score_threshold
        return predictions[filter_threshold]
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter,
                                     description="Print torchinfo.summary on Lomin OCR Detection Model")
