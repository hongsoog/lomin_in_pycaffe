...
def build_resnet_fpn_p3p7_backbone(cfg):
    ...
    logger.debug(f"\t\t# ================================")
    logger.debug(f"\t\t# 1-1-1 ResNet50 build")
    logger.debug(f"\t\t# ================================")
    ...
    logger.debug(f"\t\t# ================================")
    logger.debug(f"\t\t# 1-1-2 FPN build")
    logger.debug(f"\t\t# ================================\n")
    # get the channels parameters required by fpn
    logger.debug("\t\t# get the channels parameters required by fpn")
    ...
    logger.debug("\n\t\tfpn = fpn_module.FPN(" )
    logger.debug(f"\t\t\t\tin_channels_list = [0, {in_channels_stage2*2}, {in_channels_stage2*4}, {in_channels_stage2 * 8}],")
    logger.debug(f"\t\t\t\tout_channels = {out_channels},")
    logger.debug(f"\t\t\t\tconv_block=conv_with_kaiming_uniform( cfg.MODEL.FPN.USE_GN ={cfg.MODEL.FPN.USE_GN }, cfg.MODEL.FPN.USE_RELU ={cfg.MODEL.FPN.USE_RELU} ),")
    logger.debug(f"\t\t\t\ttop_blocks=fpn_module.LastLevelP6P7(in_channels_p6p7={in_channels_p6p7}, out_channels={out_channels},) // CALL")
...
def build_backbone(cfg):
    if logger.level == logging.DEBUG:
        ...
        logger.debug(f"\t\t// registry.BACKBONES[cfg.MODEL.BACKBONE.CONV_BODY]:")
        logger.debug(f"\t\t// {registry.BACKBONES[cfg.MODEL.BACKBONE.CONV_BODY]}\n")
